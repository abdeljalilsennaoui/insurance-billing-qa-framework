...
| Application integration | `billing-app/src/test/java/.../api/` | <!--count:integration-->141<!--/count--> | Spring context + MockMvc | HTTP contract, status codes, error shape, rendered templates |
| API automation | `qa-api-tests` | <!--count:api-->119<!--/count--> | Running application over HTTP | The published contract as a client sees it |
| UI automation | `qa-ui-tests` | <!--count:ui-->67<!--/count--> | Running application via Chrome | Browser journeys through both consoles |
| BDD | `qa-bdd-tests` | <!--count:bdd-->53<!--/count--> | Running application (API + Chrome) | Billing rules expressed as readable specifications |
| Smoke | `cypress` | <!--count:cypress-->15<!--/count--> | Running application via Cypress | Independent confirmation the console works |
| Performance | `perf` | 1 plan | Running application | Throughput and latency trend, manual |

**Total automated: <!--count:total-->571<!--/count--> tests**, of which 567 run on every pull request
...
8. **The UI suite is slow relative to its value** — roughly 2 minutes for 67 tests, because each starts a
